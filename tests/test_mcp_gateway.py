...
EXPECTED_HINTS: dict[str, tuple[bool, bool, bool, bool]] = {
    ...
    # More of the evidence that session already assembled. Read-only in the strict sense
    # the session route is not: it answers out of the held snapshot, so it builds no
    # provider request, runs no reconciliation and never opens the store at all.
    ...
    def test_the_withdraw_direction_removes_a_superseded_event_through_the_same_pair(self):
        ...
        self.tool(
            "applyCoachDecision",
            {
                ...
                "proposal": decision_prepared["proposal"],
                "confirmed": True,
            },
        )
