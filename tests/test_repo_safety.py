...
allowance to the exact published URLs and nothing wider.
...
PUBLIC_SITE_REPO_URL = f"https://github.com/{HANDLE}/paceandstaystrong-site"
PUBLIC_PAGES_URL = f"https://{HANDLE}.github.io/paceandstaystrong-site"
...
class RepoSafetyGateTest(unittest.TestCase):
    ...
    def test_published_site_may_link_back_to_its_own_repository(self):
        ...
        code, report = self.run_gate(
            {
                "README.md": f'<a href="{PUBLIC_REPO_URL}">Core</a>',
                "docs/account-lifecycle.md": (
                    f'<a href="{PUBLIC_SITE_REPO_URL}">Website</a> '
                    f"published at {PUBLIC_PAGES_URL}/."
                ),
            }
        )
    ...
    def test_the_same_handle_anywhere_else_still_fails(self):
        ...
        code, report = self.run_gate(
            {"README.md": f"Written by {HANDLE}, in Taipei."}
        )
    ...
    def test_the_owner_name_is_never_allowed(self):
        ...
        code, report = self.run_gate(
            {"README.md": f"Built by {OWNER_NAME}. {PUBLIC_REPO_URL}"}
        )
        self.assertEqual(code, 1)
