...
-- and for a long time only one did. The Custom GPT entry pasted it into the Builder;
...
- **The Custom GPT entry pastes it** -- `scripts/custom_gpt_release.py` reads this same
  file at the released commit and binds its digest into the release identity.
...
It also has a hard size limit that is not this product's: the Custom GPT Builder refuses
instructions near 8000 characters, and this file is what gets pasted there. A new
paragraph therefore costs an old one (tests/test_openapi_contract.py holds the budget).
"""
...
def instructions() -> str:
    """The orchestration text, exactly as the Custom GPT Builder receives it.

    Trailing newlines are stripped because the Builder strips them on save: the released
    digest has to be of the value the product can actually read back, not of Git's
    conventional final newline (``scripts/custom_gpt_release.py`` does the same).
    """
