...
预览会先说明这次要写进哪一个 Intervals.icu 账号——先讲 Intervals 上的 email，再讲显示名称，所以同时有正式账号和测试账号的人，在写进去之前就看得出来是哪一个。email 放前面，是因为自己的两个账号常常取同一个显示名称。两者都是当下向 Intervals 读的，只用在这一次回答，不会被存下来。
...
- **其他**：把同一个网址配置成 remote Streamable HTTP MCP server。跑在你自己机器上的 client（OAuth callback 落在 loopback）可以直接连；跑在云主机、用自己域名接 callback 的 client 也能自行注册连上，只是在进到 Intervals 授权之前，会先看到本产品自己的一页确认画面，上面写明授权会被送到哪一个 origin，点击 Continue 才会继续。该 origin 若已被运维者验证并加进列表，就直接跳过这一页。细节见 [entrypoints/mcp/README.md](entrypoints/mcp/README.md)。

逐入口「已完整实机验证」或「已封装、等待真实连接验证」的状态，以 [entrypoints/](entrypoints/README.md) 为准。

### 自建怎么跑？

Repo 使用 Python 3.11，只有一个钉死版本的依赖：官方 MCP Python SDK（`requirements.txt` 写版本，`requirements.lock` 连同它解析出的每个包一起钉死 sha256），它负责 `/mcp` 的协议与传输层。教练判断、验证、store、交付与身份都仍然只用标准库。

1. Clone repo，然后 `python3 -m pip install --require-hashes -r requirements.lock`。
...
- 托管版会按日期保存你提供的睡眠分数、睡眠时长、昨夜 HRV 与静息心率，直到你删除账号数据。教练每次读取最近 28 天，这不是保存期限；其他传入的恢复数字只供当次判断。要拿掉某一天的恢复记录，说那天的记录不算即可，但那一天是整笔拿掉，不能只留其中一个数字，详见 [数据说明](docs/data-sources.md#consequence-for-the-coach)。
...
目前 release 对外有 **23 个 MCP tool**、**2 个 prompt**、**34 个 CLI 指令**、**4 份 JSON Schema contract**、**10 张 identity 表**。这些数量由测试从真实代码推导，避免这份文档自己走样。
