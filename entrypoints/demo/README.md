...
| 409 | `turn_limit_reached` | this conversation has spent its turns |
...
| 502 / 503 / 504 | `model_unavailable`, `demo_model_unconfigured`, `model_timeout` | the turn could not be answered |
...
`store` is false on every call, so a visitor's sentence is not retained at the provider.
...
A second Railway service in the same project, pointed at this repository:

- **Config-as-code path:** `railway.demo.toml`
- **Dockerfile:** `Dockerfile.demo` (declared in that file)
- **Variables:** `OPENAI_API_KEY`, and any preview origin
- **No volume.** Nothing here persists, and a mounted volume would be the first step
  towards a second store.
- **No Intervals OAuth secrets.** It has no provider client to give them to.

The production MCP service keeps its own `railway.toml`, `Dockerfile` and volume, and none
of them are touched by any of this. Do not point `mcp.paceandstaystrong.com` at the demo, and
do not add an anonymous route to the gateway to serve this traffic.
